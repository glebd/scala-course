...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    // https://class.coursera.org/reactive-001/forum/thread?thread_id=981#comment-2654
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      val lst = Promise[List[T]]()
      lst.success(Nil)
      fs.foldRight(lst.future) {
        (f, acc) =>
          for {
            x <- f
            xs <- acc
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case Failure(e) => try p success cont(f) catch {
          case NonFatal(t) => p failure t
        }
        case Success(v) => try p success cont(f) catch {
          case NonFatal(t) => p failure t
        }
      }
      p.future
